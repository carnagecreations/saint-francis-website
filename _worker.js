...
var animalStatusEnum = pgEnum("animal_status", [
  "available",
  "fostered",
  "adopted",
  "resident"
]);
var auctionStatusEnum = pgEnum("auction_status", ["active", "closed", "won"]);
var contactSubjectEnum = pgEnum("contact_subject", [
  "adopt",
  "foster",
  "volunteer",
  "donate",
...
var auctionItemsTable = pgTable("auction_items", {
  id: serial("id").primaryKey(),
  ...
  donor: text("donor"),
  bidInstructions: text("bid_instructions"),
...
function serializeAnimal(a2) {
  const imageUrls = a2.imageUrls && a2.imageUrls.length > 0 ? a2.imageUrls : a2.imageUrl ? [a2.imageUrl] : [];
  return { ...a2, imageUrls, createdAt: a2.createdAt.toISOString() };
}
function serializeAuctionItem(i) {
  return {
    ...i,
...
app.get("/animals/featured", async (c) => {
  const db = getDb(c.env);
  const animals = await db.select().from(animalsTable).where(eq(animalsTable.isFeatured, true));
  return c.json(animals.map(serializeAnimal));
});
app.get("/animals/:id", async (c) => {
  const id = Number(c.req.param("id"));
  if (isNaN(id)) return c.json({ error: "Invalid animal ID" }, 400);
  const db = getDb(c.env);
...
app.get("/animals", async (c) => {
  ...
  if (status && status !== "all") {
    conditions.push(
      eq(animalsTable.status, status)
    );
  }
  if (species) {
    conditions.push(eq(animalsTable.species, species));
  }
  const animals = await db.select().from(animalsTable).where(conditions.length > 0 ? and(...conditions) : void 0).orderBy(animalsTable.createdAt);
  return c.json(animals.map(serializeAnimal));
});
...
app.patch("/contacts/:id", async (c) => {
  ...
  const id = Number(c.req.param("id"));
  if (isNaN(id)) return c.json({ error: "Invalid contact ID" }, 400);
...
app.post("/suggestions", async (c) => {
  ...
  }).returning({ id: suggestionsTable.id, name: suggestionsTable.name, emoji: suggestionsTable.emoji, thumbsUp: suggestionsTable.thumbsUp, thumbsDown: suggestionsTable.thumbsDown, createdAt: suggestionsTable.createdAt });
  return c.json({
    id: suggestion.id,
    name: suggestion.name,
    emoji: suggestion.emoji,
    thumbsUp: suggestion.thumbsUp,
    thumbsDown: suggestion.thumbsDown,
...
app.get("/suggestions", async (c) => {
  const db = getDb(c.env);
  const suggestions = await db.select().from(suggestionsTable).orderBy(sql`${suggestionsTable.createdAt} DESC`);
...
app.patch("/suggestions/:id/vote", async (c) => {
  ...
  const body = await c.req.json().catch(() => null);
  if (!body || !body.direction || !["up", "down"].includes(body.direction)) {
  ...
  const updates = body.direction === "up"
    ? { thumbsUp: suggestion.thumbsUp + 1 }
    : { thumbsDown: suggestion.thumbsDown + 1 };

  const updated = await db.update(suggestionsTable).set(updates).where(eq(suggestionsTable.id, id)).returning();
  return c.json({
    id: updated[0].id,
    name: updated[0].name,
    emoji: updated[0].emoji,
    thumbsUp: updated[0].thumbsUp,
    thumbsDown: updated[0].thumbsDown,
...
app.get("/fundraiser/items", async (c) => {
  const status = c.req.query("status");
  const db = getDb(c.env);
  const items = await db.select().from(auctionItemsTable).where(
    status && status !== "all" ? eq(
      auctionItemsTable.status,
      status
    ) : void 0
...
var ANIMAL_STATUSES = ["available", "fostered", "adopted", "resident"];
function isAnimalStatus(v2) {
  return typeof v2 === "string" && ANIMAL_STATUSES.includes(v2);
}
function normalizeImageUrls(body) {
  const raw2 = Array.isArray(body?.imageUrls) ? body.imageUrls : body?.imageUrl ? [body.imageUrl] : [];
...
app.post("/admin/uploads", async (c) => {
  ...
  if (!c.env.MEDIA) {
    return c.json(
      {
        error: "Image storage isn't configured. Add an R2 bucket binding named 'MEDIA' to this Pages project."
      },
      500
    );
  }
  ...
  for (const file of files) {
    if (!file.type.startsWith("image/")) {
      return c.json({ error: `"${file.name}" is not an image file` }, 400);
    }
    if (file.size > MAX_BYTES) {
      return c.json({ error: `"${file.name}" is larger than 10 MB` }, 400);
    }
    const ext = (file.name.split(".").pop() || "jpg").toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 5) || "jpg";
    const key = `animals/${Date.now()}-${crypto.randomUUID()}.${ext}`;
...
app.get("/admin/animals", async (c) => {
  if (!requireAdmin(c.env, c.req.header("Authorization"))) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const status = c.req.query("status");
  const db = getDb(c.env);
  const animals = await db.select().from(animalsTable).where(
    status && isAnimalStatus(status) ? eq(animalsTable.status, status) : void 0
  ).orderBy(animalsTable.createdAt);
  return c.json(animals.map(serializeAnimal));
});
app.post("/admin/animals", async (c) => {
  if (!requireAdmin(c.env, c.req.header("Authorization"))) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const body = await c.req.json().catch(() => null);
  const imageUrls = normalizeImageUrls(body);
  if (!body?.name || imageUrls.length === 0) {
    return c.json(
      { error: "Name and at least one photo are required" },
      400
    );
  }
  if (body.status != null && !isAnimalStatus(body.status)) {
    return c.json({ error: "Invalid status" }, 400);
  }
  const db = getDb(c.env);
  ...
  return c.json(serializeAnimal(animal), 201);
});
app.patch("/admin/animals/:id", async (c) => {
  if (!requireAdmin(c.env, c.req.header("Authorization"))) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const id = Number(c.req.param("id"));
...
app.delete("/admin/animals/:id", async (c) => {
  if (!requireAdmin(c.env, c.req.header("Authorization"))) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const id = Number(c.req.param("id"));
  if (isNaN(id)) return c.json({ error: "Invalid ID" }, 400);
  const db = getDb(c.env);
  const [deleted] = await db.delete(animalsTable).where(eq(animalsTable.id, id)).returning({ id: animalsTable.id });
  if (!deleted) return c.json({ error: "Animal not found" }, 404);
  return c.json({ id: deleted.id, message: "Animal deleted successfully" });
});
app.get("/admin/auction-items", async (c) => {
  if (!requireAdmin(c.env, c.req.header("Authorization"))) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const db = getDb(c.env);
...
app.post("/admin/auction-items", async (c) => {
  ...
  const [item] = await db.insert(auctionItemsTable).values({
    ...
    startingBidCents: Number(body.startingBidCents),
    estimatedValueCents: body.estimatedValueCents ?? null,
    donor: body.donor ?? null,
    bidInstructions: body.bidInstructions ?? null,
    endsAt: body.endsAt ? new Date(body.endsAt) : null,
    status: body.status ?? "active"
  }).returning();
  return c.json(serializeAuctionItem(item), 201);
});
app.patch("/admin/auction-items/:id", async (c) => {
  if (!requireAdmin(c.env, c.req.header("Authorization"))) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const id = Number(c.req.param("id"));
  if (isNaN(id)) return c.json({ error: "Invalid ID" }, 400);
  const body = await c.req.json().catch(() => null);
  if (!body) return c.json({ error: "Invalid item data" }, 400);
  const db = getDb(c.env);
  const [item] = await db.update(auctionItemsTable).set({
    title: body.title,
    description: body.description,
    imageUrl: body.imageUrl,
    startingBidCents: Number(body.startingBidCents),
    ...
    endsAt: body.endsAt ? new Date(body.endsAt) : null,
    status: body.status
  }).where(eq(auctionItemsTable.id, id)).returning();
  if (!item) return c.json({ error: "Auction item not found" }, 404);
  return c.json(serializeAuctionItem(item));
});
app.delete("/admin/auction-items/:id", async (c) => {
  if (!requireAdmin(c.env, c.req.header("Authorization"))) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const id = Number(c.req.param("id"));
  if (isNaN(id)) return c.json({ error: "Invalid ID" }, 400);
  const db = getDb(c.env);
  const [deleted] = await db.delete(auctionItemsTable).where(eq(auctionItemsTable.id, id)).returning({ id: auctionItemsTable.id });
...
<header class="nav">
  <div class="wrap nav-inner">
    ...
    <nav class="nav-links" id="primary-nav" aria-label="Primary">
      ...
      <a href="/adopt-apply.html" data-nav="adopt-apply">Adopt</a>
      ...
      <div class="nav-dropdown-wrapper">
        ...
        <div class="nav-dropdown-menu">
          ...
          <a href="/foster-apply.html">Foster an Animal</a>
    ...
    <div class="foot-grid">
      ...
      <div>
        ...
        <div class="foot-links">
          ...
          <a href="/ways-to-help.html">Ways to Help</a>
          <a href="/adopt-apply.html">Adopt an Animal</a>
          <a href="/foster-apply.html">Foster an Animal</a>
...
var worker_entry_default = {
  async fetch(request, env, ctx) {
    ...
    if (BLOCKED_ASSETS.test(url.pathname)) {
